...
constexpr uint8_t VELOCIDAD_MOTOR_EN_ESQUINA_PORCENTAJE = 60;
constexpr uint8_t VELOCIDAD_MINIMA_PARED_LATERAL_PORCENTAJE = 60;
...
// Despues de detectar la tercera vuelta conserva el control normal durante
// este tiempo efectivo de avance. No cuenta pausas ni retrocesos de seguridad.
constexpr unsigned long DURACION_AVANCE_FINAL_MS = 500;
...
constexpr uint16_t DISTANCIA_FRENTE_INICIO_MM = 600;
constexpr uint16_t DISTANCIA_FRENTE_CRITICA_MM = 200;

// Escape frontal: frena, centra el servo y retrocede hasta recuperar espacio.
constexpr uint16_t DISTANCIA_FRENTE_RETROCESO_MM = 150;
constexpr uint16_t DISTANCIA_FRENTE_SALIDA_RETROCESO_MM = 250;
...
// ------------------- GIRO EXPERIMENTAL DE 90 GRADOS -----------------
// S2 es el sensor izquierdo a 25 grados y S4 el derecho. El modo se arma
// cuando uno de ellos ve la pared central y comienza el giro cuando ese mismo
// sensor supera 150 cm y S3 confirma que el robot llego a una esquina.
constexpr bool GIRO_90_HABILITADO = true;
constexpr uint16_t PARED_PRESENTE_PARA_ARMAR_GIRO_MM = 800;
constexpr uint16_t SENSOR_LIBRE_PARA_INICIAR_GIRO_MM = 1250;
constexpr uint16_t DISTANCIA_FRONTAL_MAXIMA_PARA_GIRO_MM = 800;
constexpr uint8_t MUESTRAS_LIBRES_PARA_CONFIRMAR_GIRO = 3;
constexpr float ANGULO_OBJETIVO_GIRO_GRADOS = 90.0f;
constexpr float TOLERANCIA_GIRO_GRADOS = 2.0f;
constexpr int CORRECCION_SERVO_DURANTE_GIRO_GRADOS = 35;
constexpr int DIRECCION_FISICA_SERVO_GIRO = -1;
constexpr uint8_t VELOCIDAD_DURANTE_GIRO_PORCENTAJE = 55;
constexpr unsigned long DURACION_RECTA_DESPUES_GIRO_MS = 300;
constexpr unsigned long DURACION_MAXIMA_GIRO_90_MS = 2500;

// En este BNO085 el recorrido antihorario observado acumula grados negativos.
// S2 libre inicia un giro antihorario (-1); S4 libre, uno horario (+1).
constexpr int DIRECCION_GIRO_CON_S2 = -1;
constexpr int DIRECCION_GIRO_CON_S4 = 1;
...
static_assert(DURACION_AVANCE_FINAL_MS > 0,
              "El avance final debe durar mas de cero");
...
static_assert(SENSOR_LIBRE_PARA_INICIAR_GIRO_MM >
                  PARED_PRESENTE_PARA_ARMAR_GIRO_MM,
              "El umbral libre debe superar al de pared presente");
static_assert(MUESTRAS_LIBRES_PARA_CONFIRMAR_GIRO > 0,
              "El giro necesita al menos una muestra libre");
static_assert(DIRECCION_GIRO_CON_S2 == -1 &&
                  DIRECCION_GIRO_CON_S4 == 1,
              "Las direcciones de giro deben ser -1 para S2 y 1 para S4");
static_assert(DIRECCION_FISICA_SERVO_GIRO == 1 ||
                  DIRECCION_FISICA_SERVO_GIRO == -1,
              "La direccion fisica del servo debe ser 1 o -1");
...
enum class GyroCornerState : uint8_t {
  IDLE,
  TURNING,
  STRAIGHTENING,
  FAILED
...
GyroCornerState gyroCornerState = GyroCornerState::IDLE;
bool gyroCornerLeftArmed = false;
bool gyroCornerRightArmed = false;
uint8_t gyroCornerFreeSamples = 0;
int gyroCornerPendingDirection = 0;
int gyroCornerCourseDirection = 0;
int gyroCornerTurnDirection = 0;
float gyroCornerTargetTurnDegrees = 0.0f;
unsigned long gyroCornerTurnStartMs = 0;
unsigned long gyroCornerStraightStartMs = 0;
...
const char *dashboardStateText() {
  ...
  if (gyroCornerState == GyroCornerState::TURNING) {
    return "Giro de esquina por BNO085";
  }
  if (gyroCornerState == GyroCornerState::STRAIGHTENING) {
    return "Salida recta de esquina";
  }
  if (gyroCornerState == GyroCornerState::FAILED) {
    return "Falla en giro de 90 grados";
  }
...
const char *dashboardStateLevel() {
  if (missionComplete || distanceSensorFailsafeActive ||
      gyroCornerState == GyroCornerState::FAILED ||
      (EXIGIR_BNO085_PARA_MOVERSE && bno085FailsafeActive)) return "stop";
  if (finalAdvanceActive || finalReverseActive ||
      gyroCornerState != GyroCornerState::IDLE || !bno085HasHeading ||
...
void startFinalAdvance(unsigned long nowMs) {
  ...
  Serial.print(DURACION_AVANCE_FINAL_MS);
  Serial.println(" ms EFECTIVOS ***");
}
...
void updateFinalAdvance(unsigned long nowMs) {
  ...
  if (finalAdvanceAccumulatedMs >= DURACION_AVANCE_FINAL_MS) {
...
void resetGyroCornerDetection() {
  gyroCornerState = GyroCornerState::IDLE;
  gyroCornerLeftArmed = false;
  gyroCornerRightArmed = false;
  gyroCornerFreeSamples = 0;
  gyroCornerPendingDirection = 0;
  gyroCornerTurnDirection = 0;
  gyroCornerTargetTurnDegrees = 0.0f;
  gyroCornerTurnStartMs = 0;
  gyroCornerStraightStartMs = 0;
}

void startGyroCornerTurn(int turnDirection, unsigned long nowMs) {
  gyroCornerState = GyroCornerState::TURNING;
  gyroCornerTurnDirection = turnDirection;
  gyroCornerCourseDirection = turnDirection;
  gyroCornerTargetTurnDegrees =
      accumulatedTurnDegrees +
      static_cast<float>(turnDirection) * ANGULO_OBJETIVO_GIRO_GRADOS;
  gyroCornerTurnStartMs = nowMs;
  gyroCornerFreeSamples = 0;
  gyroCornerPendingDirection = 0;
  resetPidControl();

  Serial.print("Giro BNO085 iniciado: ");
  Serial.print(turnDirection == DIRECCION_GIRO_CON_S2 ? "S2" : "S4");
  Serial.print(" libre, objetivo acumulado ");
  Serial.print(gyroCornerTargetTurnDegrees, 1);
  Serial.println(" grados.");
}

bool updateGyroCornerControl(
    const uint16_t distancesMm[SENSOR_COUNT],
    unsigned long nowMs
) {
  if (!GIRO_90_HABILITADO || missionComplete || finalAdvanceActive ||
      finalReverseActive) {
    return false;
  }
  ...
  if (!bno085HasHeading || bno085FailsafeActive) {
    if (gyroCornerState != GyroCornerState::IDLE) {
      gyroCornerState = GyroCornerState::FAILED;
      stopMotor();
      writeServoAngle(SERVO_CENTRO_GRADOS);
      Serial.println("GIRO 90 detenido: no hay rumbo valido del BNO085.");
      return true;
    }
    return false;
  }
  ...
  if (gyroCornerState == GyroCornerState::TURNING) {
    const float remainingDegrees =
        static_cast<float>(gyroCornerTurnDirection) *
        (gyroCornerTargetTurnDegrees - accumulatedTurnDegrees);

    if (remainingDegrees <= TOLERANCIA_GIRO_GRADOS) {
      gyroCornerState = GyroCornerState::STRAIGHTENING;
      gyroCornerStraightStartMs = nowMs;
      writeServoAngle(SERVO_CENTRO_GRADOS);
      setMotorSpeedPercent(VELOCIDAD_DURANTE_GIRO_PORCENTAJE);
      resetPidControl();

      Serial.print("Giro de 90 grados completo en ");
      Serial.print(accumulatedTurnDegrees, 1);
      Serial.println(" grados; servo centrado.");
      return true;
    }

    if (nowMs - gyroCornerTurnStartMs >= DURACION_MAXIMA_GIRO_90_MS) {
      gyroCornerState = GyroCornerState::FAILED;
      stopMotor();
      writeServoAngle(SERVO_CENTRO_GRADOS);
      Serial.println(
          "GIRO 90 detenido: no alcanzo el angulo dentro del tiempo limite."
      );
      return true;
    }

    // DIRECCION_FISICA_SERVO_GIRO permite invertir el varillaje sin cambiar
    // el signo de giro que debe alcanzar el BNO085.
    const int servoAngle =
        SERVO_CENTRO_GRADOS -
        DIRECCION_FISICA_SERVO_GIRO * gyroCornerTurnDirection *
            CORRECCION_SERVO_DURANTE_GIRO_GRADOS;
  ...
  if (gyroCornerState == GyroCornerState::STRAIGHTENING) {
    ...
    if (nowMs - gyroCornerStraightStartMs >=
        DURACION_RECTA_DESPUES_GIRO_MS) {
      resetGyroCornerDetection();
      resetPidControl();
      Serial.println("Salida recta terminada: regresa el control PID.");
      return false;
    }
    return true;
  }

  const uint16_t s2 = distancesMm[1];
  const uint16_t s3 = distancesMm[2];
  const uint16_t s4 = distancesMm[3];
  const bool validS2 = s2 != INVALID_DISTANCE_MM;
  const bool validS3 = s3 != INVALID_DISTANCE_MM;
  const bool validS4 = s4 != INVALID_DISTANCE_MM;

  if (gyroCornerCourseDirection <= 0 && validS2 &&
      s2 <= PARED_PRESENTE_PARA_ARMAR_GIRO_MM) {
    gyroCornerLeftArmed = true;
  }
  if (gyroCornerCourseDirection >= 0 && validS4 &&
      s4 <= PARED_PRESENTE_PARA_ARMAR_GIRO_MM) {
    gyroCornerRightArmed = true;
  }

  const bool frontConfirmsCorner =
      validS3 && s3 <= DISTANCIA_FRONTAL_MAXIMA_PARA_GIRO_MM;
  const bool leftTurnCandidate =
      gyroCornerCourseDirection <= 0 && gyroCornerLeftArmed && validS2 &&
      s2 >= SENSOR_LIBRE_PARA_INICIAR_GIRO_MM && frontConfirmsCorner;
  const bool rightTurnCandidate =
      gyroCornerCourseDirection >= 0 && gyroCornerRightArmed && validS4 &&
      s4 >= SENSOR_LIBRE_PARA_INICIAR_GIRO_MM && frontConfirmsCorner;

  int candidateDirection = 0;
  if (leftTurnCandidate) {
    candidateDirection = DIRECCION_GIRO_CON_S2;
  } else if (rightTurnCandidate) {
    candidateDirection = DIRECCION_GIRO_CON_S4;
  }
  ...
  if (candidateDirection != gyroCornerPendingDirection) {
    gyroCornerPendingDirection = candidateDirection;
    gyroCornerFreeSamples = 1;
  } else if (gyroCornerFreeSamples < MUESTRAS_LIBRES_PARA_CONFIRMAR_GIRO) {
    ++gyroCornerFreeSamples;
  }

  if (gyroCornerFreeSamples >= MUESTRAS_LIBRES_PARA_CONFIRMAR_GIRO) {
    startGyroCornerTurn(candidateDirection, nowMs);
    const int servoAngle =
        SERVO_CENTRO_GRADOS -
        DIRECCION_FISICA_SERVO_GIRO * candidateDirection *
            CORRECCION_SERVO_DURANTE_GIRO_GRADOS;
    writeServoAngle(servoAngle);
    setMotorSpeedPercent(VELOCIDAD_DURANTE_GIRO_PORCENTAJE);
    return true;
  ...
  if (nowMs - lastPollMs >= SENSOR_POLL_INTERVAL_MS) {
    ...
    if (readDistancesMm(distancesMm)) {
      ...
      if (reversing) {
        if (gyroCornerState != GyroCornerState::IDLE) {
          resetGyroCornerDetection();
          Serial.println("Giro de 90 cancelado por escape frontal.");
        }
      ...
      } else if (updateGyroCornerControl(distancesMm, nowMs)) {
        updateMotorSoftStart(nowMs);

        if (nowMs - lastPrintMs >= SERIAL_PRINT_INTERVAL_MS) {
          lastPrintMs = nowMs;
          printDistances(distancesMm);
          Serial.print("Control=GIRO_90_BNO | giro=");
          Serial.print(accumulatedTurnDegrees, 1);
